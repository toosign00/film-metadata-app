...
type ProcessingCallback = (processing: boolean) => void;
 ...
 * ZIP 스트리밍 설정 객체
 */
const STREAMING_OPTIONS: StreamingOptions = {
  batchSize: 10,
...
/**
 * 단일 파일 다운로드 함수
 * @param image - 다운로드할 이미지 객체
 */
export const downloadFile = (image: ProcessedImage): void => {
  try {
    if (!image || !image.url) {
    ...
    alert(`파일 다운로드 중 오류가 발생했습니다: ${error instanceof Error ? error.message : String(error)}`);
  }
};
 ...
 * @param fileCount - 압축된 파일 개수
 */
const downloadZipFile = (blob: Blob, fileCount: number): void => {
  try {
    const now = new Date();
    ...
    alert(`ZIP 파일 다운로드 중 오류가 발생했습니다: ${error instanceof Error ? error.message : String(error)}`);
  }
};

/**
 * 여러 이미지를 ZIP으로 압축하여 다운로드합니다
 * @param validImages - 다운로드할 이미지 객체 배열
 * @param updateZipProgress - 압축 진행률 업데이트 콜백 (0-100)
 * @param updateProcessing - 처리 상태 업데이트 콜백
 * @param updateIsZipCompressing - 압축 상태 업데이트 콜백
 */
export const createZipFile = async (
  validImages: ProcessedImage[],
  updateZipProgress: ProgressCallback,
  updateProcessing: ProcessingCallback,
  updateIsZipCompressing: ProcessingCallback
): Promise<void> => {
  ...
  try {
    ...
    return new Promise<void>((resolve, reject) => {
      if (!worker) {
        reject(new Error('Worker 초기화 실패'));
        return;
      }
     ...
     * @param image - 이미지 객체
     */
    async function processFile(image: ProcessedImage): Promise<void> {
      try {
        if (!worker) return;
    ...
    function finishDownload(): void {
      try {
        ...
        handleError(`ZIP 파일 생성 중 오류: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  } catch (error) {
